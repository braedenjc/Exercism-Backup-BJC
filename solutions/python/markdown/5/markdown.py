...
def parse(markdown):
    ...
    for i in lines:
        ...
        in_list_mode = are_we_in_list_mode(i)
        if(in_list_mode):
        ...
        if m: # If there is a match from the <li> detecting regex, we are now appending the either a <ul> or <li> element.
            ...
            if not in_list: #Since we are not exactly in <li> mode yet, we will check to see if we need to bold/italicize text.
                ...
                if is_bold:
                    translate_bold_markdown(curr)
                if is_italic:
                    translate_italic_markdown(curr)
                ...
                if is_bold:
                    curr = translate_bold_markdown(curr)
                if is_italic:
                    curr = translate_italic_markdown(curr)
                i = '<li>' + curr + '</li>'
...
def are_we_in_list_mode(linetocheck):
    return linetocheck.split("* ")[0].count == 0

#split the list markdown, and exactly once.
def split_list_markdown(linetocheck):
    return linetocheck.split("* ", 1)

#This helper combines two helpers into one handy function for parsing our markdown for italic and bold.
def translate_bold_markdown(linetocheck):
    split_bold_text = split_bold_markdown(linetocheck)
    return split_bold_text[0] + "<strong>" + split_bold_text[1] + "</strong>" + split_bold_text[2]

def translate_italic_markdown(linetocheck):
    split_italic_text = split_italic_markdown(linetocheck)
    return split_italic_text[0] + '<em>' + split_italic_text[1] + '</em>' + split_italic_text[2]
